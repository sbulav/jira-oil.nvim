...
M.show_help = function(keymaps)
  ...
  for _, entry in ipairs(keymap_entries) do
    local line = string.format(" %s   %s", pad_align(entry.str, max_lhs), entry.desc)
  ...
  local ns = vim.api.nvim_create_namespace("JiraOil")
  ...
  local winid = vim.api.nvim_open_win(bufnr, true, {
    ...
    zindex = 150,
    style = "minimal",
    border = config.options.keymaps_help.border,
  })
